...
```
##Q25 transformations: Why do we need sqrt and log values of temp?  are the results of independet ttest correct?
...
```
#Q49 paired t-test with two values?
```{r corals}
#load corals
corals <- read.csv(file="data/prac-01/corals.csv", header=T)

#boxplot 
ggplot(corals, aes(time, size, color=Treatment)) + geom_boxplot()
```
